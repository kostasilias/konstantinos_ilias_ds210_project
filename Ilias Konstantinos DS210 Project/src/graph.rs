...
mod tests {
    ...
    fn test_compute_degree() {
        ...
        assert_eq!(degree.get(&1), Some(&1));
        assert_eq!(degree.get(&2), Some(&2));
        assert_eq!(degree.get(&3), Some(&2));
        assert_eq!(degree.get(&4), Some(&1));
    }
    ...
    fn test_compute_betweenness() {
        let edges = vec![(1, 2), (2, 3), (3, 4)];
        let nodes: HashSet<_> = vec![1, 2, 3, 4].into_iter().collect();
        let between = compute_betweenness(&edges, &nodes);

        let center_node = between[&2];
        let outer_node = between[&1];
        assert!(center_node > outer_node);
    }
}